    ...
    <div className="min-h-screen bg-[#0b121e] text-slate-100 font-sans pb-12 antialiased guest-layout">
      <header className="sticky top-0 z-50 bg-[#0b121e]/90 backdrop-blur-md border-b border-slate-800/80 px-5 py-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button type="button" onClick={onLogin} className="bg-[#F59E0B] hover:bg-amber-300 text-slate-950 font-bold px-4 py-1.5 rounded-full text-xs transition-all shadow-md shadow-amber-500/20 flex items-center gap-1">
      ...
      <nav className="flex items-center gap-6 px-6 py-3 border-b border-slate-800/50 text-xs font-medium text-slate-400 overflow-x-auto" aria-label="Guest navigation">
        ...
        <button type="button" onClick={onExplore} className="text-white font-semibold border-b-2 border-amber-500 pb-1 whitespace-nowrap">{t("discover")}</button>
      ...
      <main className="max-w-md mx-auto px-4 pt-6 space-y-6">
        <section className="space-y-1.5" aria-labelledby="guest-discover-title">
          <p className="eyebrow text-amber-500 flex items-center gap-2"><Sparkles size={13} /> {t("exploreNetwork")}</p>
        ...
        <button type="button" onClick={onExplore} className="relative w-full text-left group">
          ...
          <span className="block w-full bg-[#141d2b] border border-slate-800 rounded-2xl pl-10 pr-4 py-3 text-xs text-slate-500 group-hover:border-amber-500/60 transition-all">{t("searchSignals")}</span>
        </button>

        <div className="space-y-2" aria-label="Discover filters">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none text-xs">
            <button type="button" onClick={onExplore} className="px-3.5 py-1.5 rounded-full font-semibold border border-amber-500 text-amber-500 bg-amber-950/30 whitespace-nowrap">{t("need")}</button>
        ...
        <section className="bg-[#131c2a]/90 border border-slate-800/90 rounded-3xl p-5 shadow-2xl backdrop-blur-xl space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="flex items-center gap-1.5 text-[11px] font-medium text-amber-300 bg-amber-950/40 border border-amber-500/20 px-2.5 py-0.5 rounded-full"><span className="w-1.5 h-1.5 rounded-full bg-amber-300" /> {t("explore")}</span>
          </div>
