  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <form className="request-modal auth-modal" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="auth-title">
        ...
        <label className="input-label"><span>Password</span><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={mode === "sign-in" ? "current-password" : "new-password"} minLength={6} required /></label>
        <button className="primary-btn primary-btn--wide" type="submit" disabled={pending}>{pending ? <Loader2 className="spin" size={16} /> : mode === "sign-in" ? "Sign in" : "Create account"}<ArrowRight size={16} /></button>
        <button type="button" className="auth-mode-toggle" onClick={() => setMode(mode === "sign-in" ? "sign-up" : "sign-in")}>
