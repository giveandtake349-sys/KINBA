...
export const appRouter = router({
  ...
  profile: router({
    ...
    content: protectedProcedure
      ...
      .query(({ ctx, input }) => listProfileContent(ctx.user.id, input.tab)),
    get: publicProcedure
  ...
  signals: router({
    list: publicProcedure
      ...
      .query(({ ctx, input }) =>
        listSignals({ ...input, viewerId: ctx.user?.id })
      ),
    mine: protectedProcedure.query(({ ctx }) => listOwnSignals(ctx.user.id)),
    create: protectedProcedure
      .input(signalInput)
      .mutation(({ ctx, input }) => createSignal(ctx.user.id, input)),
  }),
  comments: router({
    list: publicProcedure
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (await areUsersBlocked(ctx.user.id, signal.userId))
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Comments are not available for this post.",
          });
  ...
  matches: router({
    list: protectedProcedure
      .input(
        z
          .object({ search: z.string().trim().min(2).max(100).optional() })
          .optional()
      )
      .query(({ ctx, input }) => listMatches(ctx.user.id, input?.search)),
  }),
  connections: router({
    ...
    request: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (await areUsersBlocked(ctx.user.id, input.recipientId))
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "This connection is not available.",
          });
        if (!(await getUserById(input.recipientId)))
          throw new TRPCError({
            code: "NOT_FOUND",
        ...
        if (input.signalId) {
          ...
          if (
            !signal ||
            signal.userId !== input.recipientId ||
            signal.status !== "active"
          ) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "This signal is no longer available for connection.",
            });
          }
        }
        try {
          return await createConnection(ctx.user.id, input);
        } catch (error) {
          throw new TRPCError({
            code: "CONFLICT",
            message:
    ...
    accept: protectedProcedure
      .input(positiveId)
      .mutation(async ({ ctx, input }) => {
        const connection = await getConnectionForParticipant(
          input.id,
          ctx.user.id
        );
        if (
          !connection ||
          connection.recipientId !== ctx.user.id ||
        ...
        if (
          await areUsersBlocked(connection.requesterId, connection.recipientId)
        )
          throw new TRPCError({
            code: "FORBIDDEN",
    ...
    cancel: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          !connection ||
          connection.requesterId !== ctx.user.id ||
          connection.status !== "pending"
        )
          throw new TRPCError({
            code: "FORBIDDEN",
  ...
  messages: router({
    list: protectedProcedure.input(positiveId).query(async ({ ctx, input }) => {
      ...
      if (!connection || !isAcceptedParticipant(connection, ctx.user.id))
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Messaging is available after the connection is accepted.",
        });
      if (await areUsersBlocked(connection.requesterId, connection.recipientId))
        throw new TRPCError({
          code: "FORBIDDEN",
