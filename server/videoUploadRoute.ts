...
import { MAX_LONG_VIDEO_DURATION_SECONDS, MAX_SHORT_VIDEO_DURATION_SECONDS } from "./mediaValidation";
...
async function probeVideo(filePath: string) {
  ...
  const result = JSON.parse(stdout) as { format?: { duration?: string }; streams?: { width?: number; height?: number }[] };
  const stream = result.streams?.find(item => item.width && item.height);
  const duration = Number(result.format?.duration);
  if (!Number.isFinite(duration) || !stream?.width || !stream.height) throw new Error("The video metadata could not be read.");
  return { durationSeconds: Math.ceil(duration), width: stream.width, height: stream.height };
}
...
async function authenticate(request: Request) {
  ...
  await upsertUser({ openId, name: supabaseDisplayName(supabaseUser), email: supabaseUser.email ?? null, loginMethod: "supabase", lastSignedIn: new Date() });
  ...
  app.post("/api/videos/upload", upload.single("video"), async (req, res) => {
    ...
    try {
      ...
      if (metadata.durationSeconds > maximum) {
        res.status(400).json({ error: `Videos must be ${maximum / 60 === 1 ? "1 minute" : "30 minutes"} or shorter.` });
        return;
      }
      const uploaded = await storagePut(`videos/${user.id}/source-${Date.now()}-${req.file.originalname}`, await fs.readFile(temporaryPath), req.file.mimetype);
      const sourceUrl = new URL(uploaded.url, `${req.protocol}://${req.get("host")}`).toString();
      const video = await createVideo(user.id, { title, description, videoUrl: sourceUrl, thumbnailUrl: null, kind, durationSeconds: metadata.durationSeconds, width: metadata.width, height: metadata.height, sources: [{ quality: "ORIGINAL", videoUrl: sourceUrl }] });
